const SUPPORTED_LANGUAGES = ['zh', 'en', 'fi', 'de']
...
/**
 * 设置当前语言。不支持的语言会回退为 'en'。
 * @param {string} language - 目标语言代码
 * @returns {string} 实际设置的语言代码
 */
...
/**
 * 根据当前语言查找翻译键对应的文本。
 * 查找优先级：当前语言 > 英文兜底 > 返回 key 本身。
 * 支持 {{paramName}} 模板插值。
 * @param {string} key - 翻译键
 * @param {Record<string, string|number>} [params] - 模板参数
 * @returns {string} 翻译后的文本
 */
export function t(key, params) {
  const safeParams = params ?? {}
  const template = messages[currentLanguage]?.[key] ?? messages.en[key] ?? key
  return Object.entries(safeParams).reduce(
